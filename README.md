...
Second, the allowed SASL mechanisms are `plain login`. This may be edited within dovecot's `config/conf.d/10-auth.conf` (the location and name of this file may also be altered within `config/conf.d/10-auth.conf`). All users currently are authenticated with the same password (but we all currently authenticate under president anyways). Entries within the users file follow the format of `example@example.com:{BLF-CRYPT}passwd`, where BLF-CRYPT is the password storage scheme and passwd is the encrypted password. Note that if one wishes for more SASL mechanisms, the passwords may have to be stored in plaintext. By using TLS and only plain and login SASL, the passwords can be stored with BLF-CRYPT.
